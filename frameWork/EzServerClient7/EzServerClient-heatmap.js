...
if (typeof EzServerClient.Layer === 'undefined') {
  EzServerClient.Layer = EzServerClient.Class;
}

var HeatmapOverlay = EzServerClient.Layer.extend({

  initialize: function (config) {
    this.cfg = config;
    this._el = EzServerClient.DomUtil.create('div', 'leaflet-zoom-hide');
    this._data = [];
    this._max = 1;
    this._min = 0;
  ...
  setData: function(data) {
    ...
    while (len--) {
      var entry = data[len];
      var latlng = new EzServerClient.LatLng(entry[latField], entry[lngField]);
  ...
  _resetOrigin: function () {
    this._origin = this._map.layerPointToLatLng(new EzServerClient.Point(0, 0));
    this._draw();
  } 
});
